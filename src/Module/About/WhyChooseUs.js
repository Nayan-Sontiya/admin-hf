...
const WhyChooseUs = () => {
   ...
   let hashConfig = {
    ...
    async function getAboutUsData() {
        let resp = await GetRequest("getpagecontent/why_choose_us");
        if(resp.length !== 0){
    ...
    const aboutContentAdd = async (e) => {
        let item = { page_name : 'why_choose_us', content: content }
        let resp = await PostRequest("addwebpage", item);
        if(resp._id !== undefined){
            swal({
                title: 'Success',
                text: 'Why choose us data added successfully!',
                icon: 'success',
                button: 'Ok',
            })
        }
    ...
    return (
        ...
        <div className="content-inner">
          ...
          <Heading headingText="Why choose us content" buttonComponent='' />
                ...
                <div className="widget has-shadow">
                  <form action="" >
                    <div className="d-flex justify-content-between align-items-center">
                      <div>
                       <h4 className='my-4 ml-3'>Why choose us content editor</h4>
                      </div>
